export const API_HOST = "http://localhost:4200";

// ================================================================================
// KEYCHAINS
// ================================================================================

export const API_KEYCHAINS_LIST = `${API_HOST}/api/keychains/list`;

export const API_KEYCHAINS_VIEW = `${API_HOST}/api/keychains/view`;

export const API_KEYCHAINS_CREATE = `${API_HOST}/api/keychains/create`;

export const API_KEYCHAINS_UPDATE = `${API_HOST}/api/keychains/update`;

export const API_KEYCHAINS_DELETE = `${API_HOST}/api/keychains/delete`;

// ================================================================================
// KEYCOPIES
// ================================================================================

export const API_KEYCOPIES_LIST_BY_KEYCHAIN = `${API_HOST}/api/keycopies/list-by-keychain`;

export const API_KEYCOPIES_CREATE = `${API_HOST}/api/keycopies/create`;

export const API_KEYCOPIES_DELETE = `${API_HOST}/api/keycopies/delete`;

export const API_KEYCOPIES_UPDATE = `${API_HOST}/api/keycopies/update`;

// ================================================================================
// STAFFS
// ================================================================================

export const API_STAFFS_LIST = `${API_HOST}/api/staffs/list`;

export const API_STAFFS_VIEW = `${API_HOST}/api/staffs/view`;

export const API_STAFFS_CREATE = `${API_HOST}/api/staffs/create`;

export const API_STAFFS_UPDATE = `${API_HOST}/api/staffs/update`;

export const API_STAFFS_DELETE = `${API_HOST}/api/staffs/delete`;
